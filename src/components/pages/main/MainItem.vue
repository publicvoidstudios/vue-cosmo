<template>
  ...
  >
    ...
    <div class="row g-0">
      ...
      <div
        class="col-md-5 d-flex align-items-start justify-content-center p-0 overflow-hidden bg-light-subtle rounded-start">
        <img
      ...
      <div class="col-md-7 bg-light-subtle rounded-end">
        <div class="card-body d-flex flex-column justify-content-between h-100 mt-3">
          ...
          <div class="container d-flex justify-content-evenly my-3">
            <a class="btn btn-primary m-1" :class='{"disabled" : this.inPreviewMode}'
               :data-bs-target="'#moreModal_' + html_id" @click.prevent="handleToggleMoreModal">Подробнее</a>
            <a class="btn btn-secondary m-1" :class='{"disabled" : this.inPreviewMode}'
               :data-bs-target="'#reviewsModal_' + html_id" @click.prevent="handleToggleReviewsModal">Отзывы</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal fade" :id="'reviewsModal_' + html_id" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body text-start">
            <div v-if="reviews.length" v-for="review in reviews"
                 class="container my-3 p-1 bg-light-subtle rounded shadow-sm">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" :data-bs-target="'#createReviewModal' + html_id"
                    data-bs-toggle="modal">Оставить отзыв
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="modal fade" :id="'createReviewModal' + html_id" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable" style="max-width: 1000px;">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <form>
              ...
              <div class="mb-3">
                ...
                <div id="userNameDesc" class="mt-2">
                  ...
                  <p class="small m-0 p-0">Воздержитесь от использования ненормативной лексики. Такие отзывы не будут
                    опубликованы.</p>
                </div>
              </div>
              <div class="mb-3">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" :data-bs-target="'#createdReviewModal' + html_id"
                    data-bs-toggle="modal" @click="createReview(html_id, review.user_name, review.body, review.rating)">
              Оставить отзыв
            </button>
          </div>
        </div>
    ...
    <div class="modal fade" :id="'createdReviewModal' + html_id" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable" style="max-width: 1000px;">
        <div class="modal-content">
          <div class="modal-header">
    ...
    <div class="modal fade" :id="'moreModal_' + html_id" tabindex="-1" :aria-labelledby="'moreModalLabel' + html_id"
         aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable" style="max-width: 1000px;">
        <div class="modal-content">
          <div class="modal-header">
...
export default {
  ...
  methods: {
    ...
    async createReview(parent_html_id, user_name, body, rating) {
      await fetch('http://cosmmedic.ru:3000/api/create-review', {
        method: 'POST',
        headers: {
    ...
    handleToggleMoreModal(event) {
      const modalElement = document.getElementById('moreModal_' + this.html_id);
      this.showModal(modalElement);
    },
    handleToggleReviewsModal(event) {
      const modalElement = document.getElementById('reviewsModal_' + this.html_id);
      this.showModal(modalElement);
    },
  },
  computed: {
