  ...
  methods: {
    async addComment(content_id, user_id, body) {
      if(body.trim(' ') !== '') {
        ...
        await fetch('http://cosmmedic.ru:3000/api/create-scomment', {
          method: 'POST',
          headers: {
    ...
    async reactOnComment(comment_id, reaction) {
      ...
      if(this.user.id !== -1) {
        ...
        await fetch('http://cosmmedic.ru:3000/api/modify-scomment', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
    ...
    async removeComment(id) {
      await fetch('http://cosmmedic.ru:3000/api/remove-scomment', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
